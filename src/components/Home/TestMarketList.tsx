...
const TestMarketCard = React.lazy(() => import("./TestMarketCard"));

const TestMarketList = ({ events }) => {
  ...
  let filteredMarkets = [];
  if (events) {
    filteredMarkets = events.flatMap((event) =>
        ...
        <SimpleGrid columns={{ base: 1, md: 3 }} spacing={5}>
          {filteredMarkets.length != 0
            ? filteredMarkets.map((market: any) => (
                <Stack key={market.marketAccount}>
                  <TestMarketCard market={market} event={market.event} />
                </Stack>
              ))
            : events &&
              events
                .flatMap((e) =>
                  e.markets.map((market) => ({ ...market, event: e }))
                )
                .map((market: any) => (
                  <Stack key={market.marketAccount}>
                    <TestMarketCard market={market} event={market.event} />
